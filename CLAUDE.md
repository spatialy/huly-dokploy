...
**huly-v7-pg (v3.2.0)** uses **PostgreSQL 17** (`postgres:17-alpine`). Upstream added PostgreSQL support in PR #10331 (Dec 2025), included in v0.7.353. The only CockroachDB-specific service is `kvs` (hulykvs), which uses `bytes` type in its DDL — resolved via `CREATE DOMAIN bytes AS bytea;` in a PostgreSQL init script. Uses `PROCEED_V7_MONGO=false` on account to skip MongoDB migration path. Saves ~1-1.5GB RAM vs CockroachDB. MongoDB is still used by `aibot` and `calendar`.
