...
minio_access_key   = "${password:16}"   → S3_ACCESS_KEY   (huly-v7-pg v3.3.0+)
minio_secret_key   = "${password:32}"   → S3_SECRET_KEY   (huly-v7-pg v3.3.0+)
ai_bot_password    = "${password:32}"   → AI_BOT_PASSWORD (huly-v7-pg v3.3.0+; aibot + love-agent)
...
- `AI_BOT_PASSWORD`, `S3_ACCESS_KEY`/`S3_SECRET_KEY`, `SIGN_CERTIFICATE_PASSWORD` — generated by Dokploy at deploy time (huly-v7-pg v3.3.0+; Coolify users generate them in `.env`)

### Hardcoded Defaults
- MinIO credentials: auto-generated in `huly-v7-pg` v3.3.0+; `minioadmin`/`minioadmin` elsewhere and as compose fallback (via `S3_ACCESS_KEY`/`S3_SECRET_KEY`)
- AI bot password: auto-generated in `huly-v7-pg` v3.3.0+; compose falls back to `password` when `AI_BOT_PASSWORD` is unset (matches pre-v3.3.0 deployments, whose bot account was created with that password)
