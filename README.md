...
Point Coolify at this repo and deploy the `coolify/huly-v7-next/` directory.
...
3. Set **Docker Compose Location** to `coolify/huly-v7-next/docker-compose.yml`
...
5. Generate secrets (run these on your server and paste the values):
   ...
   ```
6. Set `CR_DB_URL=postgres://huly:<your-CR_PASSWORD>@cockroach:26257/huly`
7. Configure SMTP (see [Configure Email](#configure-email-required-for-otp-login) below)
8. Click **Deploy**

See `coolify/huly-v7-next/.env.example` for the full list of variables and documentation.
...
Secrets (`SECRET`, `CR_PASSWORD`, `LIVEKIT_API_KEY`, etc.) are auto-generated by Coolify.

---

## Deploy with Docker Compose

The `coolify/huly-v7-next/` directory is a standard Docker Compose project that works with any orchestrator (Portainer, Dockge) or bare `docker compose`.

```bash
git clone https://github.com/spatialy/huly-dokploy.git
cd huly-dokploy/coolify/huly-v7-next
